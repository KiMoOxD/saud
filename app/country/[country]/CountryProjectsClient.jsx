...
function AnimatedStat({ value, label, formatter = (v) => v, icon: Icon }) {
    ...
    return (
        <div ref={ref} className="flex flex-col items-center justify-center text-center p-2">
            {Icon && <Icon className="w-8 h-8 mb-3 text-green-600" />}
            <div className="text-4xl font-bold bg-gradient-to-br from-green-600 to-emerald-500 bg-clip-text text-transparent tracking-tighter">
                {displayValue}
            </div>
...
export default function CountryProjectsClient({ country, projects }) {
    ...
    const getSectorStyle = (sector) => {
        ...
        return ({
            Healthcare: { badge: "bg-sky-100 text-sky-800" },
            Agriculture: { badge: "bg-green-100 text-green-800" },
            Manufacturing: { badge: "bg-amber-100 text-amber-800" },
            Technology: { badge: "bg-violet-100 text-violet-800" },
            RealEstate: { badge: "bg-rose-100 text-rose-800" },
            Energy: { badge: "bg-yellow-100 text-yellow-800" },
            Tourism: { badge: "bg-teal-100 text-teal-800" },
            Education: { badge: "bg-blue-100 text-blue-800" },
            FinancialServices: { badge: "bg-lime-100 text-lime-800" },
            Logistics: { badge: "bg-gray-100 text-gray-800" },
            HumanResources: { badge: "bg-purple-100 text-purple-800" },
            DesignInteriorDecoration: { badge: "bg-pink-100 text-pink-800" },
            Default: { badge: "bg-slate-100 text-slate-800" }
        }[styleKey] || { badge: "bg-slate-100 text-slate-800" });
    };
        ...
        <main className="min-h-screen bg-slate-50" dir="rtl">
            <section className="relative py-24 md:py-32 overflow-hidden bg-gradient-to-b from-green-50 to-slate-50">
                <div className="absolute inset-0 -z-10 opacity-60">
                    <div className="absolute top-0 left-0 w-3/4 h-1/2 bg-gradient-to-br from-emerald-50 to-transparent rounded-full blur-3xl"></div>
                    <div className="absolute bottom-0 right-0 w-1/2 h-1/2 bg-gradient-to-tl from-sky-50 to-transparent rounded-full blur-3xl"></div>
                </div>

                <div className="container mx-auto px-4 relative">
                    <motion.div initial={{ opacity: 0 }} animate={{ opacity: 1 }} className="flex items-center text-sm text-slate-500 mb-12">
                        <Link href="/" className="hover:text-green-600 transition-colors">الرئيسية</Link><span className="mx-2">/</span>
                        <Link href="/projects" className="hover:text-green-600 transition-colors">المشاريع</Link><span className="mx-2">/</span>
                        <span className="font-semibold text-green-700">{countryDisplayName}</span>
                    </motion.div>
                    ...
                    <div className="grid lg:grid-cols-5 gap-12 items-center">
                        <motion.div initial={{ opacity: 0, x: 20 }} animate={{ opacity: 1, x: 0 }} transition={{ duration: 0.7, ease: "easeOut" }} className="lg:col-span-3">
                            <h1 className="text-4xl md:text-6xl font-extrabold text-slate-800 mb-4 leading-tight">
                                استثماراتنا في <span className="text-transparent bg-clip-text bg-gradient-to-r from-green-600 to-emerald-500">{countryDisplayName}</span>
                            </h1>
                        ...
                        <motion.div initial={{ opacity: 0, scale: 0.8 }} animate={{ opacity: 1, scale: 1 }} transition={{ duration: 0.7, delay: 0.2, ease: "easeOut" }} className="lg:col-span-2 flex items-center justify-center">
                            <div className="grid grid-cols-3 gap-2 sm:gap-4 bg-white p-4 sm:p-6 rounded-2xl shadow-xl shadow-green-900/10 border border-slate-100 w-full max-w-md">
            ...
            {projects.length > 0 ? (
                        ...
                        <div className="container mx-auto px-4">
                            <div className="flex flex-col lg:flex-row gap-4 items-center justify-between">
                                <div className="relative w-full lg:max-w-sm">
                                    ...
                                    <Input placeholder="ابحث بالاسم أو الوصف..." onChange={(e) => setSearchQuery(e.target.value)} className="w-full h-12 pr-12 rounded-lg bg-slate-50 border-slate-200 focus:bg-white focus:border-green-500 transition" />
                                </div>
                                <div className="flex gap-3 sm:gap-4">
                                    <Select value={selectedSector} onValueChange={setSelectedSector}>
                                        <SelectTrigger className="w-[150px] sm:w-44"><SelectValue placeholder="اختر قطاع" /></SelectTrigger>
                                        <SelectContent>{availableSectors.map((s) => <SelectItem key={s} value={s}>{sectorNames[s] || s}</SelectItem>)}<SelectItem value="all">جميع القطاعات</SelectItem></SelectContent>
                                    </Select>
                                    <Select value={sortBy} onValueChange={setSortBy}>
                                        <SelectTrigger className="w-[150px] sm:w-44"><SelectValue placeholder="ترتيب حسب" /></SelectTrigger>
                                        <SelectContent><SelectItem value="investment">الأعلى استثماراً</SelectItem><SelectItem value="roi">الأعلى عائد</SelectItem><SelectItem value="name">الترتيب الأبجدي</SelectItem></SelectContent>
                                    </Select>
                                </div>
                            </div>
                        </div>
                                ...
                                {filteredAndSortedProjects.map((project) => (
                                    <motion.div key={project.id} variants={itemVariants} className="h-full">
                                        <Link href={`/projects/${project.id}`} className="block h-full">
                                            <motion.div whileHover={{ y: -8 }} transition={{ type: 'spring', stiffness: 200, damping: 15 }} className="group h-full p-6 rounded-2xl bg-white border border-slate-200 shadow-lg shadow-green-900/10 flex flex-col hover:ring-2 hover:ring-green-500">
                                                <div className="relative flex flex-col h-full">
                                                    ...
                                                    <p className="text-sm text-slate-500 line-clamp-2 mb-4 flex-grow">{project.description}</p>
                                                    ...
                                                    <div className="absolute -bottom-1 -left-2 text-green-600 font-semibold opacity-0 group-hover:opacity-100 transition-all duration-300 flex items-center gap-1 transform group-hover:bottom-0">
                ...
                <motion.div key="no-projects" initial={{ opacity: 0 }} animate={{ opacity: 1 }} exit={{ opacity: 0 }}>
                    <section className="py-16">
                        <div className="container mx-auto px-4">
                            <div className="text-center py-20 bg-white border border-slate-200 shadow-xl shadow-green-900/10 rounded-2xl">
                                <div className="inline-block p-5 bg-green-100 rounded-full mb-6 ring-8 ring-green-50">
                                ...
                                <Link href="/projects">
                                    <Button className="mt-8 bg-green-600 hover:bg-green-700 text-white font-semibold rounded-full px-6 py-3">
