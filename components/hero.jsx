...
import { Eye, Goal, Gem, Sparkles, ArrowDown, TrendingUp, Lightbulb, Shield, Zap, Play } from "lucide-react"
import React from "react"

// --- Helper Component: GridCard ---
// This card has the 3D tilt effect
const GridCard = ({ item, className }) => {
  ...
  const glowStyle = {
    background: `radial-gradient(circle at ${useTransform(mouseX, val => val + 'px')} ${useTransform(mouseY, val => val + 'px')}, rgba(16, 185, 129, 0.2), transparent 40%)`,
  };
  ...
  return (
    <motion.div
      ...
      className={`relative rounded-3xl bg-white/40 backdrop-blur-md border border-slate-200/80 shadow-sm transition-shadow duration-300 hover:shadow-lg ${className}`}
    >
      <motion.div style={glowStyle} className="absolute inset-0 rounded-3xl opacity-0 group-hover:opacity-100 transition-opacity duration-300" />
    ...
    <div style={{ perspective: "1000px" }}>
      <div className="grid grid-cols-2 gap-4 md:gap-6">
        {gridItems.map((item, i) => (
          <GridCard key={i} item={item} className="h-48 group" />
        ))}
      </div>
    </div>
  );
...
export default function Hero() {
  ...
  const itemVariants = {
    ...
    visible: { y: 0, opacity: 1, filter: "blur(0px)", transition: { type: "spring", stiffness: 100, damping: 20, duration: 0.8 } },
  };
    ...
    <section className="relative w-full min-h-screen py-24 md:py-32 overflow-hidden bg-slate-50" dir="rtl">
      ...
      <div className="absolute inset-0 z-0">
        <motion.div 
            ...
            style={{
                backgroundImage: 'radial-gradient(circle at 20% 20%, #A7F3D0 0%, transparent 30%), radial-gradient(circle at 80% 70%, #BAE6FD 0%, transparent 30%)'
            }}
        />
          ...
          <div className="text-center lg:text-right">
            ...
            >
              <motion.div whileHover={{ scale: 1.05, y: -2 }} whileTap={{ scale: 0.95 }}>
                {/* Ensure your BookingSystem component renders a button that can accept a className */}
                <BookingSystem className="!py-3 !px-6 !text-base !rounded-xl !shadow-lg !bg-emerald-500 hover:!bg-emerald-600" />
              </motion.div>
              
              <motion.div whileHover={{ scale: 1.05 }} whileTap={{ scale: 0.95 }}>
                <Button variant="ghost" className="text-slate-700 hover:bg-slate-200/70 text-base px-6 py-3 rounded-xl group">
